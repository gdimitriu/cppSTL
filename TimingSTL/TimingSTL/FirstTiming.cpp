...
void FirstTiming::run(void)
{
	cout<<"First attempt at timing the sort algorithm."<<endl;
	double *A,*B;
	unsigned long N;
	unsigned int reps;
	...
	while(iterations.size()<reps)
	{
		...
		while(finish==start);
		//recod the number of iterations:
	...
	cout<<"Time: "<<1000.0/iterations[reps/2]<<" ms "<<endl;
	return;
}
